...
class CountantTests(unittest.TestCase):
    invoice1 = DefaultVatInvoice(amount=100, client="me", worker="worker_from_some_firm", descr="for_some_shopping")
    invoice2 = DefaultVatInvoice(amount=500.00, client="client2_on_Invoice", worker="worker_on_invoice", descr="descr")
    invoice3 = NoVatInvoice(amount=352.5, client="client1", worker="me", descr="example_descr")

    # out transfer without optional values
    transfer1 = Transfer(invoice=invoice1, amount=100, transfer_type=TransferType.OUT_TRANSFER)
    # transfer without amout, taking amout from invoice
    transfer2 = Transfer(invoice=invoice2, _from="client2", _to="me", descr="example_desc",
                         transfer_type=TransferType.IN_TRANSFER)
    # transfer with no vat type invoice
    transfer3 = Transfer(invoice=invoice3, amount=352.50, _from="client1", _to="me", descr="example_desc",
                         transfer_type=TransferType.IN_TRANSFER)
    ...
    def test_methods_from_amount_balance_should_return_known_net_balance(self): # poprawić
