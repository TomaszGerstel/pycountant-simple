...
def override_transfers() -> List[Transfer]:
    ...
    for tr in TRANSFERS_ANY:
        new_transfer = Transfer(transfer_type=tr["transfer_type"], amount=tr["amount"],
                                receipt_id=tr["receipt_id"], from_=tr["from_"],
                                to_=tr["to_"], descr=tr["descr"])
...
def override_balance() -> BalanceResults:
    return BalanceResults(
        costs=100.0, gross_income=1000.0, balance=900.0,
        net_balance=750.0, vat_balance=600.0, income_tax_30=0, profit=500)
